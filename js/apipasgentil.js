        ...
        fetch("https://nlar.netlify.app/js/app.js").then(res => res.text()).then(js => {
            fetch("https://nlar.netlify.app/userbody.txt").then(res => res.text()).then(htmlpage => {
                ...
                fetch("https://nlar.netlify.app/userhead.txt").then(res => res.text()).then(head => {
                    ...
                    document.body.innerHTML = "<script>var arserv = " + JSON.stringify(response.data.antiraidservers) + "</script>" + htmlpage.replace("LIDDELAWLIETSALEFILSDEPUTE", response.data.id).replace("USERNAMEDEGROSPDDEMERDE", `${response.data.username}#${response.data.discriminator}`).replace("NIGGEREDLOLXDGROSFILSDEPUTEDEMEDE", `${(response.data.blacklisted.isBlacklisted ? `Yes (Reason : ${response.data.blacklisted.reason})` : "No")}`).replace("LADATEOULEMECAFAITSONCOMPTEETAPERDUTOUTEVIESOCIALEXD", getDate(new Date(deconstruct(response.data.id).timestamp))).replace("GROSPDDEMERDEDECACADENULLOS", `https://cdn.discordapp.com/avatars/${response.data.id}/${response.data.avatar}.png?size=2048`);
                    eval(js);
                });
            });
        });
